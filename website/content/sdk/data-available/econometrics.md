...
## How to Perform Panel Regressions

Before running the `openbb.econometrics.re` function to perform a panel regression, you need to set a multi-index to define both entity and time. This step is crucial for the proper execution of panel regressions.

Use the following code snippet to set a multi-index before running the `openbb.econometrics.re` function:
```python
wage_panel = wage_panel.set_index(['nr', 'year'], drop=False)
```

You can then proceed to perform the Random Effects regression as shown below:

...
- For a `pip` installation, when a new version is published: `pip install -U openbb[all]` and `pip install -U openbb[all]`
- Upgrade a cloned version of the GitHub repo with:

```console
git fetch
git pull
poetry install && make github-actions && npm install && npm run build
```
